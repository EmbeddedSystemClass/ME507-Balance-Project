 ...
 *           STM32F405 (on a Discovery, PolyDAQ2, or STMpunk board) can be configured
 *           to run at up to 168 MHz, while an STM32F401 or STM32F411 on a Nucleo can
 *           run up to 84 MHz. The core clock is set up in @c system_stm32f4xx.c. */
 ...
 *           interrupt. It is set up in @c port.c. 
 *           BUG: We are currently using a kludge to make the timer work with Nucleo
 *                boards. The SysTick is configured differently for these boards, so
 *                we set a value about 1/3 that for the Discovery/PolyDAQ2/STMpunk
 *                boards, and the timing works out OK. This should be fixed with a
 *                proper configuration of the SysTick to run at the correct frequency.
 */
#if (defined STM32F40_41xxx || defined __DOXYGEN__)
	#define configSYSTICK_CLOCK_HZ                ( 10000000UL )
#elif (defined STM32F401xx || defined STM32F411xx)
	#define configSYSTICK_CLOCK_HZ                (  3360000UL )
#else
	#error No board defined, so unable to choose configSYSTICK_CLOCK_HZ frequency
#endif
